(function ($) {
  ...
  function toggleDarkTheme() {
    ...
    if (isDarkTheme) {
      ...
      home1bgimg.style.backgroundImage = "linear-gradient(180deg, #1a1a1a, #000000)";
    } else {
      darktoggle.src = "assets/img/icon/mon-icon.png";
      home1bgimg.style.backgroundImage = "linear-gradient(180deg, #ffffff, #e6e6e6)";
      home2bgimg.style.backgroundImage = "linear-gradient(180deg, #ffffff, #e6e6e6)";
    }
  }
...
function loadAndShowModal(modalUrl, modalId) {

  if (myModalId != modalId && modalLoaded) { modalLoaded = false; }

  if (!modalLoaded) {
    // Make AJAX request to load modal content
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
          ...
          // Show the modal
          showLoadedModal(modalId);
        } else {
          console.error('Failed to load modal');
        }
      }
    };
    xhr.open('GET', modalUrl, true);
    xhr.send();
  }
}

function showLoadedModal(modalId) {
  // Trigger the modal display by simulating a click on the element with the appropriate data-bs-target
  myModalId = modalId;
  modalLoaded = true;
  document.querySelector('[data-bs-target="#' + modalId + '"]').click();
